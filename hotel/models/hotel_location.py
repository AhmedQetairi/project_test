...
class HotelLocation(models.Model):
    ...
    def create(self, vals):
        if "service_id" in vals:
            service_categ = self.env["hotel.location"].browse(
                vals.get("service_id")
            )
            vals.update({"parent_id": service_categ.product_categ_id.id})
        return super(HotelLocation, self).create(vals)
    ...
    def write(self, vals):
        if "service_id" in vals:
            ...
            vals.update({"parent_id": service_categ.product_categ_id.id})
        return super(HotelLocation, self).write(vals)
...
class HotelDistrict(models.Model):
    ...
    service_id = fields.Many2one("hotel.district", "District")
    
    child_ids = fields.One2many(
        "hotel.district", "service_id", "Service Child Categories"
    )
    product_categ_id = fields.Many2one(
        "product.category", "Product Category", delegate=True, required=True, copy=False
    )
    ...
    def create(self, vals):
        if "service_id" in vals:
            service_categ = self.env["hotel.district"].browse(
                vals.get("service_id")
            )
            vals.update({"parent_id": service_categ.product_categ_id.id})
        return super(HotelDistrict, self).create(vals)
    ...
    def write(self, vals):
        if "service_id" in vals:
            ...
            vals.update({"parent_id": service_categ.product_categ_id.id})
        return super(HotelDistrict, self).write(vals)
