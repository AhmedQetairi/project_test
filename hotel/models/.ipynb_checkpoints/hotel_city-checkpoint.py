...
class HotelCity(models.Model):
    ...
    district_parent = fields.Many2one("hotel.district", "District")
    
    service_id = fields.Many2one("hotel.city", "City")
    
    child_ids = fields.One2many(
        "hotel.city", "service_id", "Service Child Categories"
    )
    ...
    def create(self, vals):
        if "service_id" in vals:
            service_categ = self.env["hotel.city"].browse(
                vals.get("service_id")
            )
            vals.update({"parent_id": service_categ.product_categ_id.id})
        return super(HotelCity, self).create(vals)
    ...
    def write(self, vals):
        if "service_id" in vals:
            ...
            vals.update({"parent_id": service_categ.product_categ_id.id})
        return super(HotelCity, self).write(vals)
